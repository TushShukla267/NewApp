...
export class News extends Component {
  ...
  fetchMoreData = async () => {
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${country}&category=${category}&apiKey=${apikey}&page=${this.state.page + 1}&pageSize=8`;
  ...
  render() {
    ...
    return (
      <div className='container my-5'>
        <h1 style={{ margin: "auto", display: "flex", justifyContent: "center", fontFamily: "initial", fontSize: "50px", fontWeight: "bolder", backgroundColor: "yellow", width: "900px", border: "7px solid", borderColor: headingColor }}>
            ...
            <div className='row'>
              {this.state.articles.map((element) => (
                <div className="col-md-4" key={element.url}>
                  <NewsItem title={element.title ? element.title.slice(0, 45) : "not available"} description={element.description ? element.description.slice(0, 90) : "not available"} imageurl={element.urlToImage} newsurl={element.url} author={element.author} publishedAt={element.publishedAt} isDarkMode={isDarkMode} />
                  <br /><br />
                </div>
              ))}
            </div>
